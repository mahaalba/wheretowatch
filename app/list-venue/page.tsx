  ...
  const submit = async () => {
    try {
      const payload = {
        ...
        custom_tags: customTags, screens, capacity,
        food_until: foodUntil, drinks_until: drinksUntil,
        sports: Object.keys(sports).filter(k => sports[k]),
        custom_sports: customSports,
        fixtures: Object.keys(fixtures).filter(k => fixtures[k]),
        crowd_team: crowdTeam,
        booking_method: bookMethod === 'Other' ? bookOther : bookMethod,
        ticketing,
      };
      await supabase.from('venue_submissions').insert([payload]);
    } catch {
      // Advance to confirmation regardless
    }
    setStep(6);
  };
        ...
        {step === 5 && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {BOOK_METHODS.map(o => (
                <button key={o} onClick={() => setBookMethod(o)} style={optionRowStyle(bookMethod === o)}>
                  <span style={radioDotStyle(bookMethod === o)} />
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: C.white, borderTop: `1px solid ${C.border}`, boxShadow: '0 -6px 24px rgba(10,26,51,0.06)' }}>
        <div style={{ maxWidth: 660, margin: '0 auto', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            ...
            {isDone && (
              <button onClick={() => setStep(1)} style={{ background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: '12px 20px', fontFamily: FB, fontSize: 14, fontWeight: 700, color: C.navy, cursor: 'pointer' }}>Edit listing</button>
            )}
          </div>
          <div>
            {isDone ? (
